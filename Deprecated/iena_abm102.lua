...
iena_abm102_proto = Proto("iena_abm102","IENA ABM102 Protocol")
-- create a function to dissect it
function iena_abm102_proto.dissector(buffer,pinfo,tree)
    pinfo.cols.protocol = "iena_abm102" -- the name in the wirshark view
    local iena_top_subtree = tree:add(iena_abm102_proto,buffer(),"IENA Protocol Data")
  ...
  offset = offset + 2
  datasubtree:add(buffer(offset,2),"ABM_PTP_DAYS: " .. buffer(offset,2):uint())
  offset = offset + 2
  
  local abm_sbi_time = tostring(buffer(offset,6))
  local abm_wall_time = sbi_to_walltime(abm_sbi_time)
  datasubtree:add(buffer(offset,6),"ABM_SBITime: " .. os.date("!%H:%M:%S",abm_wall_time))
  offset = offset + 6
...
udp_table:add(2101,iena_abm102_proto)